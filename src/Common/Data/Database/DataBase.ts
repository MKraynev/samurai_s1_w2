import { Page } from "../Repo/Paginator/Page";
import { PageHandler } from "../Repo/Paginator/PageHandler";
import { Sorter } from "../Repo/Sort/Sorter";

export abstract class DataBase {
    abstract GetById(tableName: string, id: string): Promise<any | null>;
    abstract GetByPropName(tableName: string, propName: string, propVal: string): Promise<any | null>;
    abstract FindBetweenTwoProp(tableName: string, propName_1: string, propName_2: string, propVal: string): Promise<any | null>;
    abstract GetAll(tableName: string, sorter: Sorter<any>, pageHandler: PageHandler): Promise<[PageHandler, any[]]>;

    abstract Post(tableName: string, obj: any): Promise<any | null>;
    abstract Put(tableName: string, id: string, obj: any): Promise<any | null>;
    abstract PutProp(tableName: string, id: string, property: string, value: string | boolean| number): Promise<any| null>;
    abstract PushProp(tableName: string, id: string, property: string, value: string | boolean| number): Promise<any| null>;
    abstract Delete(tableName: string, id: string): Promise<any | null>;
    abstract DeleteAll(tableName: string): Promise<boolean>;

    abstract Count(tableName: string, sorter: Sorter<any>): Promise<number>;
    abstract RunDb(): Promise<boolean>;
}